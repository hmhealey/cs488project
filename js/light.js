...
Light.prototype.apply = function(shader) {
    if (shader && shader.linked) {
        ...
        gl.uniform4fv(shader.getUniformLocation("lightAmbient"), this.ambient);
        gl.uniform4fv(shader.getUniformLocation("lightSpecular"), this.specular);
    }
};

Light.applyNoLight = function(shader) {
    if (shader && shader.linked) {
        gl.uniform4f(shader.getUniformLocation("lightDiffuse"), 0.0, 0.0, 0.0, 1.0);
        gl.uniform4f(shader.getUniformLocation("lightAmbient"), 0.0, 0.0, 0.0, 1.0);
